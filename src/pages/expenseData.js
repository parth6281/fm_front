...
export default function ExpenseData() {
    ...
    async function confirmDelete(id) {
        const userId = localStorage.getItem("userId")
            ...
            : "";
        try {
            const { url, method } = Routes.api.deleteExpense(selectedExpense);
    ...
    const loadExpenseData = async () => {
        ...
        try {
            ...
            let rws = data.data;
            rws = rws.map(row => {
                return createData(row._id, row.title, row.categoryName, row.categoryId, row.amount, moment(row.dateOfExpense).format('YYYY-MM-DD'), row.paymentMethod,
                )
            });
            console.log(rws);
            setRows(rws);
                    ...
                    <Table sx={{}} size="small" aria-label="a dense table">
                        <TableHead>
                            <TableRow >
                                ...
                                <TableCell style={{ fontWeight: 'bold' }} align="right">Date Of Expense</TableCell>
